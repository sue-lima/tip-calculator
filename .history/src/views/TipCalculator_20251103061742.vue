  ...
  <div class="tip-calculator min-h-screen flex flex-col">
    ...
    <main class="tip-calculator__main flex-1 flex flex-col items-center justify-center">
      ...
      <div class="tip-calculator__mobile-button flex md:hidden mt-4 w-full justify-end p-5">
        <button class="tip-calculator__mobile-toggle w-12 h-12 rounded-full cursor-pointer bg-sky-950 flex items-center" @click="handlePanel">
          <component :is="betweenPanel ? ChevronLeftIcon : ChevronRightIcon" class="w-6 h-6 text-white" />
        </button>
      </div>
    </main>
...
export default {
  ...
  computed: {
    icon() {
      return this.betweenPanel ? 'ChevronLeftIcon' : 'ChevronRightIcon';
    }
  },
  methods: {
    handlePanel() {

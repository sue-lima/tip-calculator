<template>
  <div class="tipcalculator min-h-screen flex flex-col">
    ...
    <main class="flex-1 flex flex-col items-center justify-center p-6 bg-red-700">
      ...
      <div class="w-full flex flex-col transition-all duration-500 md:flex-row md:gap-6">
        ...
        <!-- RESULT -->
        <div
          class="w-full md:w-1/2 transition-transform duration-500"
          :class="{
      ...
      <!-- BOTÃO MOBILE -->
      <button v-if="!isDesktop" @click="toggleSection" class="mt-4 px-4 py-2 bg-sky-600 text-white rounded-full shadow-md">
        {{ isFormActive ? '>' : '<' }}
      </button>
    </main>
...
export default {
  ...
  setup() {
    ...
    const updateWindow = () => {
      ...
      if (isDesktop.value) activeSection.value = 'form'
    }
